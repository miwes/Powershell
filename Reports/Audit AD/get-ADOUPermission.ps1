function Get-ActiveDirectoryOUpermissions {
    ...
    # for domain object
    $domain = Get-ADObject -LDAPFilter "(objectClass=domain)"
    $acls = (Get-ACL -path "AD:$($domain.distinguishedname)").access
    $acltotal += foreach ($acl in $acls) {   
        If ($acl.IsInherited -eq $False) {  
            if ($customidentifiers | Select-string "$($acl.IdentityReference.Value)" -SimpleMatch ) {
                $IdentityReference = ($customidentifiers | Select-Object -Property $acl.IdentityReference.Value).$($acl.IdentityReference.Value)
            }
            ...
            [PSCustomObject]@{
                OrganizationalUnit = $domain.DistinguishedName
                Principal          = $IdentityReference
                Rights             = $acl.ActiveDirectoryRights
                AppliesTo          = Get-NameForGUID $acl.InheritedObjectType
                Item               = Get-NameForGUID $acl.ObjectType
    ...
    if ($acltotal.count -gt 0) {
        ...
        $acltotal | Sort-Object OrganizationalUnit, Principal, Rights, AppliesTo, Item, Access, Inheritance, InheritanceFrom, IsInherit | Export-Csv -Path $Output -Encoding UTF8 -Delimiter ';' -NoTypeInformation
    }
}

Get-ActiveDirectoryOUpermissions reportOUv2.txt
